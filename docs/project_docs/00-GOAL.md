...
- [x] Grafana 대시보드를 서비스 운영 관점(에러율·응답시간·처리량), 인프라 관점(CPU·메모리·네트워크), 로그 분석 관점(에러 로그·request_id·trace_id)으로 분리하여 재설계한다.
- [x] 에러율·응답시간 지표에 대해 단기(5분)·장기(24시간) 트렌드를 시각화하는 패널을 추가한다.
